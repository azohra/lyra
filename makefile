...
buildbins: #build on diff platform
	@env GOOS=linux GOARCH=amd64 go build -v -o ${PROJECT_NAME}-linux-amd64  ./cmd/${PROJECT_NAME}/...
	@env GOOS=darwin GOARCH=amd64 go build -v -o ${PROJECT_NAME}-darwin-amd64  ./cmd/${PROJECT_NAME}/...

clean: #Remove previous build and undo install
	@rm -fr /usr/local/bin/${PROJECT_NAME}
